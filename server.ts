...
async function main() {
  ...
  // A Server Action that ends in `redirect()` tries to self-fetch the redirect target's
  // RSC payload (so the client gets it inlined instead of a second round trip) —
  // `next/dist/esm/server/app-render/action-handler.js`'s `origin` is `initURL`'s origin
  // (derived from the *inbound* request's Host header) unless this private env var is
  // set. On Railway (and any deployment behind a reverse proxy/edge), that's the public
  // domain — the container fetching its own public URL from inside itself is a "hairpin"
  // self-connection many container networks simply refuse, surfacing as a logged (but
  // caught — Next falls back to an ordinary redirect) `failed to get redirect response
  // [TypeError: fetch failed] { code: 'ECONNREFUSED' }` on every redirecting action.
  // Pointing this at the loopback address this same process is about to `server.listen()`
  // on below sidesteps the container network entirely. Only set if not already configured,
  // so a deployment that already sets this for its own reasons isn't overridden.
